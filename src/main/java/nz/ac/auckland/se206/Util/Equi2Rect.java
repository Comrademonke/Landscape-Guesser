...
public class Equi2Rect {
  ...
  public static void initForIntArray2D(int intArray2D[][]) {
    math_setLookUp(intArray2D);
  }

  public static int[] extractRectilinear(
      double yaw,
      double pitch,
      double fov,
      int equiData[][],
    ...
    if (ai != null) {
      if (atan_LU_HR == null) {
        ...
        for (int j = 0; j < NATAN + 1; ) {
          if (j < NATAN) atan_LU[j] = Math.atan(d / (1.0D - d)) * 256D;
          else atan_LU[j] = (Math.PI / 2) * 256;
          j++;
          d += d1;
        }
      }
  ...
  private static void math_updateLookUp(int i) {
    int j = i << 6;
    if (PV_atan0_HR != j) {
      double dist_e = (double) i / (2.0 * Math.PI);
      PV_atan0_HR = j;
      PV_pi_HR = 128 * i;
      for (int k = 0; k < NATAN + 1; k++) atan_LU_HR[k] = (int) (dist_e * atan_LU[k] + 0.5D);
    }
  }
  ...
  private static void math_extractview(
    ...
    for (k = 0; k <= UNIT_XSAMPLES * 2; k++) {
      lanczos2_LU[k] = (int) (sinc(x) * sinc(x / 2.0) * UNIT_YSAMPLES + 0.5);
      x += dx;
    }
  ...
  //
  private static void math_set_int_matrix(
      double fov, double pan, double tilt, int vw, double[][] mt, long[][] mi) {
    double fovRad = Math.toRadians(fov); // field of view in rad
    double p = (double) vw / (2 * Math.tan(fovRad / 2));
    SetMatrix(Math.toRadians(tilt), Math.toRadians(pan), mt);
    ...
    double ta;
    if (fovRad <= 0.29999999999999999) {
      ta = 436906.66666666669;
    } else {
      ta = 131072.0 / fovRad;
    }

    for (int j = 0; j < 3; j++) {
      for (int k = 0; k < 3; k++) mi[j][k] = (long) (ta * mt[j][k] * MI_MULT + 0.5D);
    }
  }
  ...
  private static void math_transform(
    ...
    if (N_POINTS_INTERP_X == 0)
      N_POINTS_INTERP_X = 1; // to guarantee value cannot go to 0 and give / 0 errors.
    ...
    if (vfov_2 > 10) { // only if not zooming in too much...
      ...
      if (pitch + vfov_2 > 87.5 || pitch - vfov_2 < -87.5) {
        if (pitch > 0) {
          // looking upwards
          N_ROW_NO_INTERP_MIN =
              y_min + (int) ((y_max - y_min) * (1 - (92.5 - (pitch - vfov_2)) / (2 * vfov_2)));
          N_ROW_NO_INTERP_MAX =
              y_min + (int) ((y_max - y_min) * (1 - (87.5 - (pitch - vfov_2)) / (2 * vfov_2)));
        } else {
          N_ROW_NO_INTERP_MIN =
              y_min + (int) ((y_max - y_min) * (1 - (-87.5 - (pitch - vfov_2)) / (2 * vfov_2)));
          N_ROW_NO_INTERP_MAX =
      ...
      if (pitch + vfov_2 > 90 - angle || pitch - vfov_2 < -90 + angle) {
        if (pitch > 0) {
          // looking upwards
          N_ROW_DOUBLE_INTERP_LOW =
              y_min
                  + (int) ((y_max - y_min) * (1 - (90 + angle - (pitch - vfov_2)) / (2 * vfov_2)));
          N_ROW_DOUBLE_INTERP_HIGH =
              y_min
                  + (int) ((y_max - y_min) * (1 - (90 - angle - (pitch - vfov_2)) / (2 * vfov_2)));
        } else {
          N_ROW_DOUBLE_INTERP_LOW =
      ...
      //			System.out.println( "Low " + N_ROW_DOUBLE_INTERP_LOW + "       High " +
      // N_ROW_DOUBLE_INTERP_HIGH );
    }
    ///////////////////////////////////////////////////////////
    ...
    for (int y = y_min; y < y_max; ) {
      ...
      row_ynew[irow] =
          PV_atan2_HR(
              (int) v1 >> MI_SHIFT,
              PV_sqrt((int) Math.abs(v2 >> MI_SHIFT), (int) Math.abs(v0 >> MI_SHIFT)));
      ...
      if (firstTime) {
        // the first time only computes the first row and loops: that computation should be done
        // before the loop
        // but I didn't like the idea of duplicating so much code so I arranged the code in such a
        // way
        firstTime = false;
        continue;
      }
      ...
      for (itmp = 0; itmp < ROWS_INT_SIZE; itmp++) {
        if ((row_xnew[itmp] < -pw_shifted_3) && (row_xold[itmp] > pw_shifted_3))
          ...
          if ((row_xnew[itmp] > pw_shifted_3) && (row_xold[itmp] < -pw_shifted_3))
            row_xdelta[itmp] =
                (row_xnew[itmp] - pw_shifted - row_xold[itmp]) / (N_POINTS_INTERP_Y_P1);
          else row_xdelta[itmp] = (row_xnew[itmp] - row_xold[itmp]) / (N_POINTS_INTERP_Y_P1);
        }
      ...
      for (int ky = 0; ky < N_POINTS_INTERP_Y_P1; ky++) {
        ...
        for (int x = x_min + 1; x <= x_max; ) {
          ...
          if ((x_new < -pw_shifted_3) && (x_old > pw_shifted_3))
            ...
            if ((x_new > pw_shifted_3) && (x_old < -pw_shifted_3))
              delta_x = (x_new - pw_shifted - x_old) / (N_POINTS_INTERP_X_P1);
            else delta_x = (x_new - x_old) / (N_POINTS_INTERP_X_P1);
          }
          ...
          for (int kk = 0; kk < N_POINTS_INTERP_X_P1; kk++) {
            if (x > x_max) break;
            if (cur_x >= pw_shifted_2) cur_x -= pw_shifted;
            if (cur_x < -pw_shifted_2) cur_x += pw_shifted;
            cur_y += delta_y;
            int dx = cur_x & 0xff;
            int dy = cur_y & 0xff;
            ...
            if (nn) {
              if (dy < 128) row_index = ys_org;
              else row_index = ys_org + 1;
            ...
            if (v_idx == 0) {
              ...
              if (v_idx == 0) {
                if (nn) {
                  if (dx < 128) col_index = xs_org;
                  else col_index = xs_org + 1;
                  ...
                  if (lanczos2)
                    // v[idx] = lanczos2_interp_pixel( pd, pw, ph - deltaYHorizonPosition, xs_org,
                    // ys_org, dx, dy);
                    v[idx] =
                        lanczos2_interp_pixel(
                            pd,
                            pw,
                            ph,
                            xs_org,
                            ...
                            lanczos2_n_points,
                            lanczos2_weights_LU);
                  else v[idx] = bilinear_interp_pixel(px00, px01, px10, px11, dx, dy);
                  // hv[idx] = (byte) (px00 >> 24); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                }
              }
            }
  ...
  private static int bilinear_interp_pixel(int p00, int p01, int p10, int p11, int dx, int dy) {
    ...
    int j3 =
        i2 * (p00 >> 8 & 0xff)
                ...
                + k2 * (p11 >> 8 & 0xff)
            >> 16;
    int k3 = i2 * (p00 & 0xff) + l2 * (p01 & 0xff) + j2 * (p10 & 0xff) + k2 * (p11 & 0xff) >> 16;

    return i3 + (j3 << 8) + k3 + 0xff000000;
  }

  //
  private static void SetMatrix(double d, double d1, double ad[][]) {
  ...
  private static void matrix_matrix_mult(double ad[][], double ad1[][], double ad2[][]) {
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++)
  ...
  static int PV_atan2_HR(int pi, int pj) {
    ...
    int index;
    if (j > 0)
      if (i > 0) return atan_LU_HR[(int) ((NATAN * i) / (j + i))];
      else return -atan_LU_HR[(int) ((NATAN * -i) / (j - i))];
    if (j == 0)
      if (i > 0) return PV_atan0_HR;
      else return -PV_atan0_HR;
    if (i < 0) {
      index = (int) ((NATAN * i) / (j + i));
      return atan_LU_HR[index] - PV_pi_HR;
      //				return atan_LU_HR[(int) ((NATAN * i) / (j + i))] - PV_pi_HR;
    } else return -atan_LU_HR[(int) ((NATAN * -i) / (j - i))] + PV_pi_HR;
  }

  static int PV_sqrt(int pi, int pj) {
    long i = pi;
    long j = pj;
    if (i > j) return (int) (i * sqrt_LU[(int) ((j << NSQRT_SHIFT) / i)] >> NSQRT_SHIFT);
    if (j == 0) return 0;
    else return (int) (j * sqrt_LU[(int) ((i << NSQRT_SHIFT) / j)] >> NSQRT_SHIFT);
  }
  ...
  private static int lanczos2_compute_weights(
      double pscale, int[] lanczos2_LU, int[][] lanczos2_weights_LU) {
    double s, corr;
    ...
    for (int j = 0; j <= UNIT_XSAMPLES; j++) {
      ...
      for (k = 0; k < lanczos2_n_points; k++) {
        lanczos2_weights_LU[j][k] = lanczos2_LU[(int) (i * pscale + 0.5)];
      ...
      for (k = 0; k < lanczos2_n_points * 2; k++) {
        lanczos2_weights_LU[j][k] = (int) (lanczos2_weights_LU[j][k] * corr);
      }
    }
  ...
  static int lanczos2_interp_pixel(
    ...
    for (ki = 0; ki < np2; ki++) {
      ...
      for (kj = 0; kj < np2; kj++) {
        ...
        j2 = j;
        if (i2 < 0) i2 = -i2 - 1;
        if (i2 >= ph) i2 = ph - (i2 - ph) - 1;
        if (j2 < 0) j2 = -j2 - 1;
        if (j2 >= pw) j2 = pw - (j2 - pw) - 1;
  ...
  private static int tmpValue(int value) {
