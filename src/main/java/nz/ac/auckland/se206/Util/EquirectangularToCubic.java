...
/**
 * Generate six cubemap images which represent the six orthogonal directions of a sphere using a
 * Gnomonic Projection with 90 degree field of view. The input file is a Cylindrical Equidistant
 * Projection with a 2:1 width to height ratio. Cubemap images format: +------+ | 4 | | |
 * +------+------+------+------+ | 0 | 1 | 2 | 3 | | | | | | +------+------+------+------+ | 5 | | |
 * +------+
 */
public class EquirectangularToCubic {
   ...
   *
   * @param equi The BufferedImage containing a cylindrical equidistant projection of a spherical
   *     panorama
   */
  public static BufferedImage[] processImage(BufferedImage equi) throws IOException {
    ...
    ArrayList<Future<int[]>> rectData = new ArrayList(6);
    ...
    for (int i = 0; i < 6; i++) {
      rectData.add(
          executor.submit(
              new callableProcessor(
                  yaw[i], pitch[i], fov, equiData, rectWidth, rectHeight, equiWidth)));
    }
...
class callableProcessor implements Callable<int[]> {
  ...
  public int[] call() {
    return Equi2Rect.extractRectilinear(
        yaw, pitch, fov, equiData, rectWidth, rectHeight, equiWidth, false, true);
  }
}
